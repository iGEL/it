...
describe It::Tag do
  ...
  describe '#process' do
    it "returns a tag with the options as attributes and the param as content" do
      expect(described_class.new(:i, "id" => "cool", "class" => "classy").process("some text")).to eq('<i class="classy" id="cool">some text</i>')
    end
    ...
    it "escapes HTML" do
      expect(described_class.new(:i).process("some text & <b>html</b>")).to eq('<i>some text &amp; &lt;b&gt;html&lt;/b&gt;</i>')
    end

    it "doesn't escape strings marked as HTML safe" do
      expect(described_class.new(:i).process("some text & <b>html</b>".html_safe)).to eq('<i>some text & <b>html</b></i>')
    end
